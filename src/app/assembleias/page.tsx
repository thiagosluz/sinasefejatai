    ...
    <div className="min-h-screen bg-brand-cream text-brand-ink p-6 md:p-8 font-sans selection:bg-brand-tinto selection:text-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {!assembleias || assembleias.length === 0 ? (
          <div className="lg:col-span-2 bg-brand-card border border-zinc-350 p-12 text-center shadow-xl">
          ...
          assembleias.map((assembleia) => (
            <div key={assembleia.id} className="bg-brand-card border border-zinc-350 shadow-xl flex flex-col justify-between group">
              <div className="p-6">
                
                {/* Status e Identificador da Convocatória */}
                <div className="flex items-center justify-between mb-4 border-b border-dashed border-zinc-300 pb-3">
                  ...
                  {assembleia.numero && (
                    <span className="text-xs font-bold uppercase tracking-widest text-zinc-550">Edital nº {assembleia.numero}</span>
                  )}
                </div>
                ...
                <div className="space-y-2.5 mb-6 text-xs text-zinc-700">
                ...
                <div className="border-t border-zinc-300 pt-4">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-500 mb-2 font-serif">Pauta de Deliberações:</h3>
                  <ul className="list-disc list-inside text-xs text-zinc-650 space-y-1">
              ...
              <div className="bg-[#e9e6de] border-t border-zinc-350 p-4 flex items-center justify-between gap-4">
                <Link 
                  href={`/assembleias/${assembleia.id}/edital`}
                  className="flex-1 flex items-center justify-center gap-1.5 py-2 px-2.5 bg-brand-cream hover:bg-brand-card border border-zinc-350 text-zinc-700 transition-colors text-xs font-bold uppercase tracking-wider shadow-[1.5px_1.5px_0px_#121214] hover:shadow-[0.5px_0.5px_0px_#121214] hover:translate-x-[0.5px] hover:translate-y-[0.5px]"
                >
                  <FileText size={13} />
                  <span>Edital</span>
                </Link>
