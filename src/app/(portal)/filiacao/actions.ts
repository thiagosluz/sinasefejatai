...
export async function solicitarFiliacao(formData: FormData) {
  ...
  if (!nome || !email || !siape || !categoria || !situacao) {
    redirect('/filiacao?error=Preencha todos os campos obrigatórios')
  }
  ...
  if (error) {
    console.error('Erro ao salvar pedido de filiação:', error)
    redirect('/filiacao?error=Ocorreu um erro. Tente novamente ou entre em contato com o sindicato.')
  }
