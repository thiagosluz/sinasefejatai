...
export function PortariaLayout({ dados, numero, config, verificacao, status = 'ativo' }: PortariaLayoutProps) {
  const dataStr = formatarDataExtenso(dados.data_emissao)

  const considerandosParagrafos = (dados.considerandos || '').split('\n').filter(p => p.trim())
  const resolveParagrafos = (dados.resolve || '').split('\n').filter(p => p.trim())
    ...
    <div id="documento-print-area" className={`relative bg-white text-black p-8 sm:p-12 min-h-[29.7cm] w-full max-w-[21cm] mx-auto shadow-2xl print:shadow-none print:m-0 print:p-0 print:w-auto print:max-w-none print:min-h-0 font-serif flex flex-col overflow-hidden ${status === 'cancelado' ? 'grayscale opacity-75' : ''}`}>
      ...
      <div className="relative z-10 flex-1 flex flex-col">
        ...
        <div className="flex flex-col items-center mb-12">
          <h2 className="text-xl font-bold tracking-[0.2em] uppercase mb-2">Portaria</h2>
        ...
        <div className="text-justify text-base leading-relaxed mb-6">
          <p className="indent-10">
            <strong>{dados.emitente || 'O Coordenador Geral do SINASEFE - Seção Sindical Jataí'}</strong>, no uso de suas atribuições estatutárias,
          </p>
        </div>
        ...
        <div className="text-base leading-relaxed mb-12">
          ...
          <div className="space-y-3">
            {resolveParagrafos.length > 0 ? (
              resolveParagrafos.map((p, i) => (
                <p key={i} className="indent-10 text-justify">{p}</p>
              ))
            ) : (
              <p className="indent-10 text-zinc-400">[Artigos da portaria serão exibidos aqui]</p>
            )}
          </div>
        </div>
