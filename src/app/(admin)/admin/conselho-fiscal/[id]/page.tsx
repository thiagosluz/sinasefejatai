...
export const metadata = {
  title: 'Membros da Gestão | Conselho Fiscal',
}
...
export default async function GestaoMembrosPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AdminPageWrapper>
      <div className="mb-2">
        <Link href="/admin/conselho-fiscal" className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-brand-ink hover:text-brand-tinto transition-colors">
          <ArrowLeft size={14} /> Voltar para Gestões
        </Link>
      </div>
      <AdminPageHeader titulo={`Gestão: ${gestao.nome}`} subtitulo="Gerencie as cadeiras fixas e adicione cargos extras do conselho" />
