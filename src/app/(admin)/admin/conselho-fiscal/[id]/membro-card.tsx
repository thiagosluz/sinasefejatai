...
export default function MembroCard({ membro, filiados }: { membro: Membro, filiados: {id: string, nome: string, perfis?: {id: string}[]}[] }) {
  ...
  const handleSave = async () => {
    ...
    if (criarAcesso && !emailConvite.trim()) {
      await alert('Para criar um acesso, você precisa informar o e-mail do conselheiro.')
      return
    }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const fileName = `conselho-${membro.id}-${Date.now()}.${fileExt}`
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1 space-y-1">
              ...
              <input
                ...
                placeholder="Ex: Titular, Suplente..."
              />
              {membro.is_cargo_fixo && (
                 <p className="text-[10px] text-zinc-400">
                   Você pode flexionar o gênero, mas a cadeira permanecerá contabilizada.
                 </p>
              )}
            </div>
                ...
                <>
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-sm font-semibold text-brand-ink">
                      Criar acesso de sistema para este conselheiro?
                    </span>
                  </label>
                  ...
                  {criarAcesso && (
                    <div className="pl-6 space-y-1">
                      ...
                      <p className="text-xs text-zinc-500 mt-1">
                        O sistema criará o usuário como membro do conselho fiscal e enviará um link para ele definir a senha.
                      </p>
                    </div>
                  )}
                </>
